...
abstract class DormMealHelper {
  static int getDayIndex(DateTime dateTime) {
    // 한국 시간 기준으로 변환 (UTC+9)
    final koreaTime = getKoreaDateTime(dateTime);
    return (koreaTime.weekday - 1) % 7; // 월요일이 0, 일요일이 6
  }

  static DateTime getKoreaDateTime(DateTime dateTime) => dateTime.toUtc().add(const Duration(hours: 9));

  // 오늘 날짜의 요일 인덱스 계산 (한국 시간 기준)
  static int getCurrentDayIndex() => getDayIndex(DateTime.now());

  static MealType getCurrentAvailableMealType() {
    // 한국 시간으로 현재 시간 구하기
